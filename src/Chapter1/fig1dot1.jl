...
function fig1dot1()
    ...
    if iplot
        ...
        plt.text(-.2, 0.45, L"y=m_0(x)")
        plt.text(xval[2] - 0.1, 0.125, L"(x_1,0)")
        plt.text(xval[2] + 0.05, yval[2] - 0.1, L"(x_1,y_1)")
        plt.text(xval[3] + 0.1, -.1, L"(x_2,0)")
        plt.text(xval[3] + 0.1, yval[3], L"(x_2,y_2)")
        plt.text(-.1, -.25, L"y=m_1(x)")
        plt.text(-.07, 0.05, L"x^*")
        plot(xval[2], 0, "ko")
        plot(xval[3], 0, "ko")
        ylabel("atan(x)")
        xlabel("x")
