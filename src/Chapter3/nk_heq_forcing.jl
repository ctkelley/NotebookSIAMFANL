...
function nk_heq_forcing(printlabel = true)
    ...
    koutfe = nsoli(
        heqf!, u0, FS, FPS; pdata = hdata, eta = 0.1, fixedeta = true,
        rtol = 1.0e-8, atol = 1.0e-8
    )
    nl_stats!(plot_hist, koutfe, L"fixed $\eta = .1$"; method = :nkj)
    koutew = nsoli(
        heqf!, u0, FS, FPS; pdata = hdata, eta = 0.1,
        rtol = 1.0e-8, atol = 1.0e-8, fixedeta = false
    )
    nl_stats!(plot_hist, koutew, L"Eisenstat-Walker: $\eta_{max} = .1$"; method = :nkj)
    koutew9 = nsoli(
        heqf!, u0, FS, FPS; pdata = hdata, eta = 0.9,
        rtol = 1.0e-8, atol = 1.0e-8, fixedeta = false
    )
    nl_stats!(
        plot_hist, koutew9,
